...
namespace CIYamlGeneratorNamespace // Ensure the namespace matches the one used in QualityBuddyWindow
{
    ...
    public class CIYamlGenerator
    {
        public static string GenerateYaml(
            ...
            TriggerOption triggerOption) // Add triggerOption parameter
        {
...
& $env:UNITY_PATH -batchmode -nographics -returnlicense -logFile - || true"
            ...
            {
                name = "Unity Build",
                on = triggerOption switch
                {
                    TriggerOption.OnPush => new Trigger { push = new Branches { branches = new List<string> { "main" } } },
                    TriggerOption.OnPullRequest => new Trigger { pull_request = new Branches() },
                    TriggerOption.Manual => new Trigger { workflow_dispatch = new object() },
                    TriggerOption.Scheduled => !string.IsNullOrWhiteSpace(cronExpression)
                        ? new Trigger { schedule = new List<CronSchedule> { new CronSchedule { cron = cronExpression } } }
                        : null,
                    _ => null
                },
                jobs = jobs
